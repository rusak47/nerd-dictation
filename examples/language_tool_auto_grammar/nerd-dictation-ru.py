...
class ENDINGS:
    EMPTY = Ending("", "")
    DOT = Ending("тут точка", ".")
    QUESTION = Ending("тут вопрос","?")
    EXCLAMATION = Ending("тут ура", "!")
...
def _run_split_tests():
    cases = [
        ("Привет мир тут точка Как дела тут вопрос в начале", ENDINGS.ALL),
        ("Привет тут точка мир", ENDINGS.ALL),
        ("Привет тут вопрос мир", ENDINGS.ALL),
        ("тут точка в начале", ENDINGS.ALL),
        ("без разделителей", ENDINGS.ALL),
        ("один тут точка два тут вопрос три", ENDINGS.ALL),
    ]

 
    for text, sep in cases:
        head, s, tail = rsplit_last(text, sep)
        print(f" before:{text}")
        print(f' split_last: head:{head}; separator:{s.spoken}; tail:{tail}')

        headc, sc, tailc = ENDINGS.rsplit_last_and_replace(text)
        print(f'\n clean: head:{headc}; separator:{sc.spoken}; tail:{tailc}')

        if head:
           head += " "
 
        if tail and s is not ENDINGS.EMPTY:
           tail = " " + tail
 
        assert head+s.spoken+tail == text, (
            f"Reconstruction failed:\n"
            f"  original:      {text!r}\n"
            f"  parts:         {(head, s, tail)!r}"
        )

    print("[OK] split / reconstruct tests passed")
